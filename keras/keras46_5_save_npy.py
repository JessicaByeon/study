...
xy_train = train_datagen.flow_from_directory( # 디렉토리(폴더)에서 가져온 것을 위와 같은 조건으로 생성해서 xy_train에 집어넣겠다.
    ...
    batch_size=500, # 크게 줘도 에러는 나지 않는다. 자동으로 최대값에 맞춰 진행
...
print(xy_test[0][0].shape, xy_test[0][1].shape) # (120, 200, 200, 1) (120,)

np.save('d:/study_data/_save/_npy/keras46_5_train_x.npy', arr=xy_train[0][0]) # train x 가 들어감
np.save('d:/study_data/_save/_npy/keras46_5_train_y.npy', arr=xy_train[0][1]) # train y 가 들어감
np.save('d:/study_data/_save/_npy/keras46_5_test_x.npy', arr=xy_test[0][0]) # test x 가 들어감
np.save('d:/study_data/_save/_npy/keras46_5_test_y.npy', arr=xy_test[0][1]) # test y 가 들어감
