...
from tensorflow.python.keras.layers import Dense, LSTM, Embedding, Conv1D, Flatten # (통상 input layer쪽에서 많이 씀)

model = Sequential()        # 인풋은 (14,5) (원핫을 하지 않은 상태)
                   # 단어사전의 갯수
model.add(Embedding(input_dim=30, output_dim=10, input_length=5)) # -> 통상 3차원 수치 출력
# model.add(Embedding(input_dim=30, output_dim=10)) # -> 통상 3차원 수치 출력
# model.add(Embedding(30, 10)) # 이런 형태로도 사용 가능
# model.add(Embedding(30, 10, 5)) # ValueError: Could not interpret initializer identifier: 5
# model.add(Embedding(30, 10, input_length=5)) # input_length 명시를 해줘야 실행가능

# model.add(LSTM(32))
# model.add(Flatten())
model.add(Conv1D(32, 2)) # stride default 1
...
# 임베딩 파라미터 연산 갯수 = 전체 단어사전의 갯수(31) * 출력값(10)
# input length 의 길이를 알면 명시해주고, 몰라서 명시하지 않으면 자동으로 None 으로 (파라미터 연산 개수 연관없음)
